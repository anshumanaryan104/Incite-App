...
class CustomScrollPhysics extends PageScrollPhysics {
  final double? dragStartDistanceMotionThres, minFlingVelo, minFlingDis, maxFlingVelo, ratio, mass, stiffness;
...
class CustomScrollPhysics2 extends PageScrollPhysics {
  const CustomScrollPhysics2({super.parent});
  ...
  double applyBoundaryConditions(ScrollMetrics position, double value) {
    ...
    } else if (value > position.maxScrollExtent && position.pixels >= position.maxScrollExtent) {
  ...
  Simulation? createBallisticSimulation(ScrollMetrics position, double velocity) {
    ...
    if (newPixels < position.minScrollExtent) {
      return ScrollSpringSimulation(spring, position.pixels, position.minScrollExtent, -velocity.abs());
    } else if (newPixels > position.maxScrollExtent) {
      return ScrollSpringSimulation(spring, position.pixels, position.maxScrollExtent, velocity.abs());
    }
