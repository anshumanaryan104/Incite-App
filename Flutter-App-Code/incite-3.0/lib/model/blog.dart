...
class DataCollection {
  DataCollection({
    this.success,
    this.categories,
    this.isSubCategoryIncluded = false,
  });

  bool? success;
  bool? isSubCategoryIncluded;
...
class Category {
  ...
  bool? isFeed;
  DataModel? data;
  List<BlogSubCategory>? subCategories;
  DateTime? createdAt, updatedAt;

  factory Category.fromJson(Map<String, dynamic> map, {bool isSubCategoryIncluded = false}) {
    // Fix: Handle both 'blogs' and 'data' keys from API
    var blogData = map['data'] ?? map['blogs'];

    // Only modify if blogData exists and is a Map
    if (blogData != null && blogData is Map<String, dynamic>) {
      blogData['category_color'] = map['color'];
      blogData['category_name'] = map['name'];
    }

    return Category(
      id: map['id'] ?? 0,
      name: map['name'],
      parentId: map["parent_id"] != null ? int.parse(map["parent_id"].toString()) : 0,
      isFeatured: map["is_featured"] != null ? int.parse(map["is_featured"].toString()) : 0,
      image: map['image'] ?? '',
      color: map['color'] ?? '#000000',
      isFeed: map['is_feed'] ?? false,
      subCategories: map.containsKey('blog_sub_category')
          ? List<BlogSubCategory>.from(map['blog_sub_category'].map((e) => BlogSubCategory.fromJson(e)))
          : [],
      data: isSubCategoryIncluded == false && blogData != null ? DataModel.fromJson(blogData) : null,
    );
  }
  ...
  bool operator ==(Object other) =>
      ...
      other is Category && runtimeType == other.runtimeType && color == other.color && name == other.name;
...
class Blog {
  ...
  Blog.fromJson(Map<String, dynamic> json, {isNotification = false, isCache = false, isAds = false}) {
    ...
    isFeatured = json["is_featured"].toString() != 'null' ? int.parse(json["is_featured"].toString()) : 0;
    ...
    videoUrl = json['video_url'] != null
        ? json['video_url'].toString().contains('?si=')
            ? json['video_url'].toString().split('?si=')[0]
            : json['video_url']
        : "";
    categoryColor = isNotification
        ? json.containsKey('blog_category')
            ? json['blog_category'][0]['category']['color']
            : ''
        : json['category_color'] ?? "#000000";
    categoryName = isNotification
        ? json.containsKey('blog_category')
            ? json['blog_category'][0]['category']['name']
            : ''
        : json['category_name'] ?? "";
    isVotingEnable = isAds == true
        ? 0
        : json["is_voting_enable"].toString() != 'null'
            ? int.parse(json['is_voting_enable'].toString())
            : 0;
    postType = isAds ? PostType.ads : getPostType(json['type']);
    scheduleDate = isAds == true
        ? DateTime.now()
    ...
    isBookmark = isAds == true
        ? 0
        : json["is_bookmark"].toString() != 'null'
            ? int.parse(json['is_bookmark'].toString())
            : 0;
    isUserViewed = isAds == true
        ? 0
        : json["is_user_viewed"].toString() != 'null'
            ? int.parse(json['is_bookmark'].toString())
            : 0;
  ...
  Map<String, dynamic> toJson() {
    ...
    if (blogSubCategory != null) {
      data['blog_sub_category'] = blogSubCategory!.map((v) => v.toJson()).toList();
    }
    if (question != null) {
...
class PollOption {
  PollOption({this.id, this.option, this.percentage});
...
class DataModel {
  ...
  DataModel.fromJson(Map<String, dynamic> json,
      {bool isSearch = false, bool isShorts = false, bool fromDeepLink = false}) {
    var cat;
    currentPage = json.containsKey('current_page') ? json['current_page'] : 0;
    // Fix: Handle both 'data' and 'blogs' keys
    var blogArray = json['blogs'] ?? json['data'];
    if (blogArray != [] && blogArray != null) {
      blogs = <Blog>[];
      blogArray.forEach((v) {
      ...
      if (isShorts == true && fromDeepLink == false) {
